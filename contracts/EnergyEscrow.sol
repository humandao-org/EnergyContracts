    ...
    contract EnergyEscrow is Ownable {
        ...
        constructor(IERC20 _ENRG, address[5] memory authorizedAddresses) Ownable(msg.sender) {
            ENRG = _ENRG;
        ...
        modifier onlyOwnerOrAuthorized() {
            require(owner() == _msgSender() || isAuthorized[_msgSender()], "Caller is not authorized");
            _;
        }

        // Allow the owner to update the list of authorized addresses
        function setAuthorization(address _address, bool _status) external onlyOwner {
        ...
        function createDeposit(
            ...
            require(assistantCount > 0, "Invalid assistant count");
            Deposit storage deposit = deposits[uuid];
            require(deposit.depositor == address(0), "Deposit exists");
        ...
        function createOpenEndedDeposit(
            ...
            require(deposits[uuid].depositor == address(0), "Deposit exists");
        ...
        function increaseEnergyAmount(bytes32 uuid, uint256 amount) external {
            Deposit storage deposit = deposits[uuid];
            require(msg.sender == deposit.depositor, "Unauthorized");
            require(!deposit.isOpenEnded, "Open-ended deposit");
            require(deposit.assistantCount > 0, "Invalid assistant count");
            ...
            if (deposit.assistantCount == 1) {
                ...
                require(newClaimableAmount > deposit.claimableAmount, "Decrease in claimable amount");
        ...
        function increaseEnergyAmountForOpenEnded(
            ...
            _checkOpenEnded(deposit);
            require(claimableAmount > deposit.claimableAmount, "Lower claimable amount");
        ...
        function increaseAssistantCount(
            ...
            require(!deposit.isOpenEnded, "Deposit is open-ended");
            require(deposit.assistantCount > 1, "Invalid assistant count");
            require(assistantCount > deposit.assistantCount, "Lower assistant count");

            uint256 requiredAmount = deposit.claimableAmount * (assistantCount - deposit.assistantCount);
            require(amount == requiredAmount, "Incorrect amount");
        ...
        function setENRG(IERC20 _ENRG) external onlyOwner {
            require(address(_ENRG) != address(0), "Invalid ENRG address");
            ENRG = _ENRG;
        ...
        function setAllowRefund(bytes32 uuid, bool allow) external onlyOwner {
            require(deposits[uuid].depositor != address(0), "Deposit not found");
        ...
        function addRecipient(
            ...
            bytes32 messageHash = keccak256(abi.encodePacked(uuid, recipient, recipientUuid));
            ...
            if (!deposit.isOpenEnded) {
                require(deposit.recipients.length < deposit.assistantCount, "Max recipients reached");
            } else {
                require(deposit.amount > deposit.claimableAmount, "Insufficient balance for new recipient");
            }
         ...
         */
        function setClaimable(bytes32 uuid, bytes32 recUuid) external onlyOwnerOrAuthorized {
            Deposit storage deposit = deposits[uuid];
            uint256 index = recipientIndex[uuid][recUuid];
            require(index < deposit.recipients.length, "Recipient not found");
    ...
    /**
         ...
         */
        function bulkSetClaimable(bytes32 uuid, bytes32[] calldata recUuids) external onlyOwnerOrAuthorized {
            Deposit storage deposit = deposits[uuid];
        ...
        function claim(bytes32 uuid, bytes32 recipientUuid) external {
            Deposit storage deposit = deposits[uuid];
            require(deposit.recipients.length > 0, "No recipients");

            uint256 index = recipientIndex[uuid][recipientUuid];
            require(index < deposit.recipients.length, "Recipient not found");

            Recipient storage recipient = deposit.recipients[index];
            require(msg.sender == recipient.recipientAddress, "Unauthorized recipient");
            require(recipient.claimable, "Not claimable");
        ...
        function refund(bytes32 uuid) external {
            ...
            require(
                ...
                "Unauthorized or refund not allowed"
            );

            require(deposit.recipients.length == 0, "Recipients present");
         ...
         */
        function removeRecipient(bytes32 uuid, bytes32 recUuid) external onlyOwnerOrAuthorized() {
            Deposit storage deposit = deposits[uuid];
            _checkInvalidDeposit(deposit);
         ...
         */
        function deleteDeposit(bytes32 uuid) external onlyOwnerOrAuthorized() {
            require(deposits[uuid].amount == 0, "Deposit not empty");
        ...
        //     require(index < deposit.recipients.length, "Recipient not found");
        //     Recipient storage recipient = deposit.recipients[index];
        //     require(!recipient.claimed, "Cannot change address for claimed deposit");
        ...
        function viewRecipientUuids(bytes32 depositUuid) public view returns (bytes32[] memory) {
            Deposit storage deposit = deposits[depositUuid];
            bytes32[] memory uuids = new bytes32[](deposit.recipients.length);

            for (uint i = 0; i < deposit.recipients.length; i++) {
                uuids[i] = deposit.recipients[i].uuid;
            }
            return uuids;
        }
        ...
        function _checkInvalidDeposit(Deposit storage uniqueDeposit) internal view {
            require(uniqueDeposit.depositor != address(0), "Invalid deposit");
        }
    }
