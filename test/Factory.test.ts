...
describe("Energy Factory", async () => {
    ...
    async function deployFixture() {
        ...
        const usdcAmount: bigint = BigInt(1) * BigInt(10)**await usdcContract.decimals(); // 100000 USDC
        const usdtAmount: bigint = BigInt(1) * BigInt(10)**await usdcContract.decimals(); // 100000 USDT
        const wethAmount: bigint = BigInt(100) * BigInt(10)**await wethContract.decimals(); // 100 WETH
        const hdaoAmount: bigint = BigInt(100) * BigInt(10)**await wethContract.decimals(); // 10000 HDAO
    ...
    describe("Minting", () => {
        ...
        it("Should be able to mint using dynamic token pricing", async () => {
            ...
            expect(await hdaoContract.balanceOf(await factoryContract.getAddress())).to.be.equal(price);
        });
    });
